...
public class CBezierCurve : MonoBehaviour {
    ...
    [SerializeField] private GameObject gRelayObject1;  // 1つ目の中継点
    [SerializeField] private GameObject gRelayObject2;  // 2つ目の中継点
    [SerializeField] private GameObject gTarget;        // 終点

    private Vector3 vStartPos;                  // 初期位置
    private Vector3 vMovePoint;                 // 移動座標格納変数

    [SerializeField] private int iMiddlePoints;    // 終点に行くまでの時間(1sec = 60flame)
